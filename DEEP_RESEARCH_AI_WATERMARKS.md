# Исследование: водяные знаки в тексте LLM

**Дата проверки:** 2026-08-16
**Область:** только текстовые файлы, включая Markdown и исходный код
**Провайдеры:** Anthropic, Google и OpenAI
**Цель:** определить, что локальная CLI-утилита может обнаружить без секретных ключей провайдера

> Обнаруженная аномалия Unicode не доказывает использование ИИ, а отсутствие обнаруженного сигнала не доказывает человеческое авторство
...
#### Доступ к официальному detector API

На 16 августа 2026 года Anthropic анонсировала скорый выпуск watermark detection API, но ещё не опубликовала правила доступа, дату запуска, authentication, quotas, цену, request/response schema или условия обработки отправленного текста. Формулировка Help Center `users and other third parties` указывает на намерение дать доступ шире круга регуляторов, но не гарантирует открытую регистрацию для любого пользователя.

Anthropic подписала Section 1 добровольного EU Code of Practice. Для подписантов Code устанавливает следующую модель доступа:

- detector может быть опубликован как открытая спецификация, локальное ПО или облачный API;
- доступ по умолчанию предоставляется бесплатно;
- узкое ценовое исключение предусмотрено для подписанта с менее чем одним миллионом monthly users, существенными операционными расходами и необычно большим объёмом запросов одного пользователя; применимость этого исключения к Anthropic публично не установлена;
- регуляторы, правоохранительные органы, media organisations, fact-checkers, trusted flaggers, независимые исследователи, образовательные и исследовательские учреждения и civil society organisations должны получать бесплатный доступ без ограничения объёма запросов;
- если результатами может пользоваться широкая публика, detector по общему правилу должен быть доступен широкой публике;
- для watermark свободного текста доступ разрешено временно ограничить verified expert users, пока точность и устойчивость таких механизмов остаются недостаточными;
- при загрузке контента Code требует минимизации данных и немедленного удаления его дословной копии после проверки, кроме узких исключений для безопасности и предотвращения злоупотреблений;
- к 2 февраля 2027 года подписант должен реализовать один из нескольких вариантов interoperability. Это может быть маршрутизация API, signpost, общий detector консорциума или сопоставимое решение, а не обязательно единый центральный endpoint.

Это обязательства подписанта в рамках добровольного Code, признанного Еврокомиссией подходящим способом демонстрации compliance, а не безусловная законодательная гарантия конкретному человеку. Поэтому обычный пользователь, вероятно, получит некоторую форму доступа, но до публикации документации нельзя утверждать, что API будет открыт всем, безлимитен, примет чужие репозитории или вернёт score по абзацам. Также пока не подтверждены выдача секретного ключа, локальный offline detector и необходимость отправлять Anthropic дословный текст вместо privacy-preserving derivative.

Для `wmtext` это ничего не меняет: локальный scanner не видит token-sampling watermark. Возможный provider adapter должен быть отдельной явно сетевой и opt-in функцией только после публикации официального API и его privacy terms.

Источники:

- [EU Code of Practice, Section 1, Measures 2.1 and 3.4](https://ec.europa.eu/newsroom/dae/redirection/document/129555)
- [European Commission: signatories of the Code](https://digital-strategy.ec.europa.eu/en/news/strong-backing-code-practice-transparency-ai-generated-content)
...
Guidelines Еврокомиссии также относят source code к результатам вне области обязанности по marking в Article 50(2). Это не запрещает провайдеру добровольно маркировать код. Anthropic отдельно указывает, что watermark в коде обычно слабее из-за малого числа произвольных token choices, но может появляться, например, в комментариях.
...
- [European Commission Q&A on Article 50 transparency obligations](https://digital-strategy.ec.europa.eu/en/faqs/transparency-obligations-under-article-50-ai-act)
